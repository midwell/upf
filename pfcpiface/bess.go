...
func (b *bess) addFAR(ctx context.Context, done chan<- bool, far far) {
	go func() {
		...
		// The datapath's real answer, delivered through the guarantee that it is delivered
		// exactly once. The two halves are what make each other safe: reporting the true
		// outcome is what makes GRPCJoin's failure path reachable at all, and draining the
		// batch is what stops that path leaving an unknown subset already programmed.
